...
def roundtrip_alignment(alignment, temp_dir):
    path = Path(temp_dir) / "alignment.phy"
    AlignIO.write(alignment, str(path), "phylip")
    return AlignIO.read(str(path), "phylip")
...
def main():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        roundtrip_ms, roundtrip_alignment_result = time_call(lambda: roundtrip_alignment(alignment, temp_dir))
        ...
        print(f"python_phylip_roundtrip_ms={roundtrip_ms:.4f}")
        print(f"python_phylip_roundtrip_ok={str(True).lower()}")
