...
function main()
    ...
    try
        ...
        input_path = joinpath(temp_dir, "alignment.msf")
        BioToolkit.write_alignment(input_path, alignment, "msf")

        roundtrip_ms, roundtrip = elapsed_ms() do
            BioToolkit.read_alignment(input_path, "msf")
        end
        ...
        python_output = read(`conda run -n general --no-capture-output python $(joinpath(@__DIR__, "msf_compare.py")) $input_path`, String)

        python_roundtrip_ms = parse(Float64, match(r"python_msf_roundtrip_ms=([0-9.]+)", python_output).captures[1])
        python_roundtrip_ok = match(r"python_msf_roundtrip_ok=(true|false)", python_output).captures[1] == "true"

        @assert python_roundtrip_ok

        println("MSF round-trip benchmark")
