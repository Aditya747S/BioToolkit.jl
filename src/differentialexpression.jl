...
export CountMatrix, DEResult, AbstractGLMSolver, GLMSolver, NativeIRLSSolver, GLMjlAdapterSolver, DESeqDataSet, DESeqDataSetFromMatrix, HurdleDEResult, mast_hurdle_test, pseudobulk_by_donor_celltype, pseudobulk_de,
  ...
  volcano_to_html, export_volcano_html, ma_plot_to_html, export_ma_plot_html,
  dispersion_plot_to_html, export_dispersion_plot_html, de_heatmap_to_html, export_de_heatmap_html
...
function input_checksum(dds::DESeqDataSet)
  ...
  write(buf, dds.counts.counts.nzval)
  write(buf, dds.counts.counts.rowval)
  write(buf, dds.counts.counts.colptr)
...
@inline function _as_dense_row(mat::SparseMatrixCSC{Int,Int}, idx::Int)
  return Vector{Float64}(mat[idx, :])
end

@inline function _as_dense_row(mat::AbstractMatrix{<:Integer}, idx::Int)
  ...
  elseif design isa DataFrame
    ...
    for cname in names(design)
      ...
      if eltype(vals) <: Number
        ...
        svals = String.(vals)
        levs = unique(svals)
        length(levs) <= 1 && continue
        if modelMatrixType == :expanded
          for lev in levs
            push!(col_list, Float64.(svals .== lev))
    ...
    X = Matrix{Float64}(undef, n, length(col_list))
    @inbounds for j in eachindex(col_list)
      X[:, j] .= col_list[j]
    end
...
function _nb_loglik(y::AbstractVector{<:Real}, mu::AbstractVector{<:Real}, dispersion::Real)
  ...
  r = 1.0 / alpha
  lgr = loggamma(r)  # constant across observations
  ll = 0.0
  @inbounds for i in eachindex(y)
    mui = max(Float64(mu[i]), eps(Float64))
    yi = max(Float64(y[i]), 0.0)
    ll += loggamma(yi + r) - lgr - loggamma(yi + 1.0) +
...
function validate_design(cm::CountMatrix, design; warn_confounding::Bool=true)
  ...
  # Condition number — use fully-qualified name to avoid shadowing
  cond_num = LinearAlgebra.cond(X)
  if cond_num > 1e10
    push!(warnings, "Design matrix is ill-conditioned: κ=$(round(cond_num; sigdigits=3))")
  elseif cond_num > 1e6
    push!(warnings, "Design matrix has high condition number: κ=$(round(cond_num; sigdigits=3))")
  end
...
function calibrate_pvalues_empirical_null(pvalues::AbstractVector;
  ...
  for i in 1:n
    if !ismissing(pvalues[i]) && isfinite(Float64(pvalues[i]))
      ...
      push!(z, quantile(Normal(), 1.0 - p / 2.0))
      push!(pos, i)
    end
  end
...
function kde_spline(x::AbstractVector{<:Real}, grid::AbstractVector{<:Real}; df::Int=7)
  ...
  std_x = std(x)
  h = std_x > 0.0 ? 1.06 * std_x * n^(-0.2) : 1.0  # Silverman's rule with fallback
  inv_h2 = -0.5 / (h * h)
...
function _fit_gene_dispersion_mle(y::AbstractVector{<:Real},
  ...
  solver::Union{Nothing,GLMSolver}=nothing,
  alpha_init::Real=0.1,
  alpha_min::Real=1e-8,
  ...
  solver = solver === nothing ? GLMSolver(X) : solver
  ...
  function objective(logalpha)
    ...
    _, logabsdet_fisher = logabsdet(Symmetric(solver.xtwx))
    if !isfinite(logabsdet_fisher)
      _, logabsdet_fisher = logabsdet(Symmetric(solver.xtwx + 1e-8I(size(solver.xtwx, 1))))
    end
    if !isfinite(logabsdet_fisher)
      return Inf
    end
...
function estimate_dispersions_prior(cm::CountMatrix,
  ...
  if model_matrix === nothing
    ...
    # Pre-compute means (no dependency on MLE)
    for g in 1:ng
      row = @view counts_dense[g, :]
      yn = row ./ nf
      means[g] = max(mean(yn), 1e-8)
    ...
    fallback_counts = zeros(Int, nthreads)
    ...
    Threads.@threads for g in 1:ng
      ...
      if !isfinite(alpha_g) || alpha_g <= 0.0
        fallback_counts[tid] += 1
        alpha_g = _moment_dispersion(row, nf)
      end
      gene_wise[g] = clamp(alpha_g, 1e-8, max_disp)
    end
    n_gene_mle_fallback = sum(fallback_counts)
    n_gene_mle_error = sum(error_counts)
  end
...
function fit_gene_fast!(solver::GLMSolver,
  ...
  beta_prev = zeros(Float64, p)  # pre-allocated buffer

  for iter in 1:maxit
    solver.iterations = iter
    copyto!(beta_prev, solver.beta)
...
function nbinomLRT(dds::DESeqDataSet;
  ...
  solver_full_pool = [GLMSolver(full_X) for _ in 1:nthreads]
  solver_red_pool = [GLMSolver(red_X) for _ in 1:nthreads]
  offset = log.(nf)
...
function _squeeze_variances(var_g::Vector{Float64}, df_resid::Int)
  ...
  count_valid < 3 && return (copy(var_g), 0.0, fill(median(var_g[valid]), length(var_g)))
  ...
  Threads.@threads for g in 1:ng
    ...
    function objective(beta_coef)
      ...
      @inbounds for i in 1:ns
        ll += loggamma(y[i] + r) - loggamma(r) - loggamma(y[i] + 1.0) +
    ...
    try
      lo = min(-5.0 * scale, x - 5.0 * scale)
      hi = max(5.0 * scale, x + 5.0 * scale)
      opt = Optim.optimize(objective, lo, hi, Optim.Brent(); iterations=max_iter)
  ...
  Threads.@threads for g in 1:ng
    ...
    if alpha < 1e-10
      @inbounds for s in 1:ns
        ...
        out[g, s] = 2.0 * sqrt(x) * inv_log2
      end
    else
      inv_sqrt_alpha_2 = (2.0 / sqrt(alpha)) * inv_log2
      @inbounds for s in 1:ns
        x = max(Y[g, s] / nf[s], 0.0)
        out[g, s] = inv_sqrt_alpha_2 * asinh(sqrt(alpha * x))
      end
    end
  end
...
function _ensure_sample_design(design::AbstractVector, nsamples::Int)
  length(design) == nsamples || throw(ArgumentError("design length must equal number of samples"))
  B, _ = _one_hot_labels(string.(design))
  return B
end
...
function sv_analysis(cm::CountMatrix, bio_design;
  ...
  prev_sv = nothing
  significant = Int[]  # lifted to outer scope
  ...
  for iter in 1:max_iter
    ...
    for k in 1:kmax
      ...
      null_cor = Float64[]
      for _ in 1:nperm
        perm_resid = reduce(hcat, [resid[:, g][randperm(rng, ns)] for g in 1:ng])
  ...
  Threads.@threads for g in 1:ng
    ...
    for iter in 1:max_iter
      ...
      function neg_weighted_nb_loglik(log_alpha)
        ...
        for s in 1:ns
          ll += w_nb[s] * (
            loggamma(y[s] + r_try) - loggamma(r_try) - loggamma(y[s] + 1.0) +
      ...
      for s in 1:ns
        if y[s] == 0
          ...
          loglik += log(1 - pi_new) + loggamma(y[s] + r_new) - loggamma(r_new) - loggamma(y[s] + 1.0) +
...
function time_series_de(cm::CountMatrix, timepoints, condition;
  ...
  nthreads = max(Threads.nthreads(), Threads.maxthreadid())
  solver_full_pool = [GLMSolver(X_full) for _ in 1:nthreads]
...
function bootstrap_lfc(cm::CountMatrix, design;
  ...
  # Fit original model & bootstrap per gene in parallel
  ...
  Threads.@threads for g in 1:ng
    ...
    alpha = max(disp[g], 1e-8)
    r = 1.0 / alpha
    y_boot = zeros(Float64, ns)

    for b in 1:n_boot
      for s in 1:ns
        p = r / (r + mu_g[s])
        y_boot[s] = Float64(rand(local_rng, NegativeBinomial(r, p)))
      end
      try
        beta_b, _, _ = fit_gene_fast!(solver, y_boot, offset, disp[g]; maxit=100)
        lfc_boot[g, b] = beta_b[coef_idx] / log(2)
      catch
        lfc_boot[g, b] = NaN
      end
    end
...
<head>
  ...
  <style>
    ...
    h1 { font-size: 1.5rem; font-weight: 600; color: #f8fafc; margin-bottom: 16px; text-align: center; }
    .plot-container { background: #1e293b; border: 1px solid #334155; border-radius: 12px; padding: 20px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.3); max-width: 1000px; margin: 0 auto; }
  </style>
</head>
<body>
...
function _to_json_val(x::Any)
  if x isa Number
    return isfinite(Float64(x)) ? string(x) : "null"
  elseif x isa AbstractString
    s = String(x)
    s = replace(s, "\\" => "\\\\", "\"" => "\\\"", "\n" => "\\n", "\r" => "\\r", "\t" => "\\t")
    return "\"$s\""
  elseif x isa Symbol
    return _to_json_val(string(x))
  elseif x isa Bool
    return x ? "true" : "false"
  elseif x isa AbstractVector
    return string("[", join([_to_json_val(v) for v in x], ","), "]")
  elseif x isa AbstractDict
    pairs = ["$(_to_json_val(string(k))):$(_to_json_val(v))" for (k, v) in x]
    return string("{", join(pairs, ","), "}")
  elseif ismissing(x) || x === nothing
    return "null"
  else
    return _to_json_val(string(x))
  end
...
"""
    volcano_to_html(df_or_results; log2fc_col=:log2_fold_change, pval_col=:padj, lfc_thresh=1.0, p_thresh=0.05, ...)

Generate an interactive Plotly HTML volcano plot for differential expression results.
"""
function volcano_to_html(df_or_results;
  log2fc_col::Symbol=:log2_fold_change,
  pval_col::Symbol=:padj,
  lfc_thresh::Real=1.0,
  p_thresh::Real=0.05,
  title::String="Differential Expression Volcano Plot",
  dom_id::String="volcano-plot-" * string(rand(1000:9999)),
  standalone::Bool=true)

  tab = df_or_results isa DataFrame ? df_or_results : DataFrame(df_or_results)

  gene_ids = String.(tab[!, :gene_id])
  log2fc = Float64[(ismissing(v) || !isfinite(Float64(v))) ? NaN : Float64(v) for v in tab[!, log2fc_col]]
  pvals = Float64[(ismissing(v) || !isfinite(Float64(v))) ? NaN : Float64(v) for v in tab[!, pval_col]]

  valid_mask = isfinite.(log2fc) .& isfinite.(pvals)
  g_valid = gene_ids[valid_mask]
  l_valid = log2fc[valid_mask]
  p_valid = pvals[valid_mask]
  nlog10_p = -log10.(clamp.(p_valid, 1e-300, 1.0))

  up_idx = findall(i -> l_valid[i] >= lfc_thresh && p_valid[i] <= p_thresh, 1:length(g_valid))
  down_idx = findall(i -> l_valid[i] <= -lfc_thresh && p_valid[i] <= p_thresh, 1:length(g_valid))
  ns_idx = findall(i -> !(i in up_idx) && !(i in down_idx), 1:length(g_valid))

  make_trace(indices, name, color) = Dict(
    "x" => l_valid[indices],
    "y" => nlog10_p[indices],
    "text" => ["Gene: $(g_valid[i])<br>log2FC: $(round(l_valid[i], digits=3))<br>p-val: $(@sprintf("%.2e", p_valid[i]))" for i in indices],
    "mode" => "markers",
    "type" => "scatter",
    "name" => name,
  ...
  traces = [
    make_trace(up_idx, "Up-regulated ($(length(up_idx)))", "#ef4444"),
    make_trace(down_idx, "Down-regulated ($(length(down_idx)))", "#3b82f6"),
    make_trace(ns_idx, "Not Significant ($(length(ns_idx)))", "#6b7280")
  ]

  p_cutoff_nlog = -log10(clamp(Float64(p_thresh), 1e-300, 1.0))
  min_x = isempty(l_valid) ? -3.0 : floor(minimum(l_valid) - 0.5)
  max_x = isempty(l_valid) ? 3.0 : ceil(maximum(l_valid) + 0.5)

  layout = Dict(
    "title" => Dict("text" => title, "font" => Dict("color" => "#f8fafc", "size" => 18)),
    "paper_bgcolor" => "#1e293b",
    "plot_bgcolor" => "#0f172a",
    "xaxis" => Dict("title" => "log2 Fold Change", "gridcolor" => "#334155", "zerolinecolor" => "#475569", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc")),
    "yaxis" => Dict("title" => "-log10(p-value)", "gridcolor" => "#334155", "zerolinecolor" => "#475569", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc")),
    "hovermode" => "closest",
    "legend" => Dict("font" => Dict("color" => "#f8fafc")),
    "shapes" => [
      Dict("type" => "line", "x0" => min_x, "x1" => max_x, "y0" => p_cutoff_nlog, "y1" => p_cutoff_nlog, "line" => Dict("color" => "#f59e0b", "width" => 1.5, "dash" => "dash")),
      Dict("type" => "line", "x0" => lfc_thresh, "x1" => lfc_thresh, "y0" => 0, "y1" => isempty(nlog10_p) ? 10 : maximum(nlog10_p) * 1.05, "line" => Dict("color" => "#94a3b8", "width" => 1, "dash" => "dot")),
      Dict("type" => "line", "x0" => -lfc_thresh, "x1" => -lfc_thresh, "y0" => 0, "y1" => isempty(nlog10_p) ? 10 : maximum(nlog10_p) * 1.05, "line" => Dict("color" => "#94a3b8", "width" => 1, "dash" => "dot"))
    ]
  )

  clean_dom = _escape_html_text(dom_id)
  html_snippet = """
<div id="$(clean_dom)" style="width:100%; height:550px;"></div>
<script src="https://cdn.plot.ly/plotly-2.27.0.min.js"></script>
<script>
  (function() {
    var data = $(_to_json_val(traces));
    var layout = $(_to_json_val(layout));
    Plotly.newPlot('$(clean_dom)', data, layout, {responsive: true});
  })();
</script>
"""
  return _wrap_html_document(html_snippet, title; standalone=standalone)
end

function export_volcano_html(filepath::String, df_or_results; kwargs...)
  content = volcano_to_html(df_or_results; standalone=true, kwargs...)
...
"""
    ma_plot_to_html(df_or_results; basemean_col=:base_mean, log2fc_col=:log2_fold_change, pval_col=:padj, ...)

Generate an interactive Plotly HTML MA plot (log10 baseMean vs log2FC).
"""
function ma_plot_to_html(df_or_results;
  basemean_col::Symbol=:base_mean,
  log2fc_col::Symbol=:log2_fold_change,
  pval_col::Symbol=:padj,
  p_thresh::Real=0.05,
  title::String="Differential Expression MA Plot",
  dom_id::String="ma-plot-" * string(rand(1000:9999)),
  standalone::Bool=true)

  tab = df_or_results isa DataFrame ? df_or_results : DataFrame(df_or_results)

  gene_ids = String.(tab[!, :gene_id])
  base_means = Float64[(ismissing(v) || !isfinite(Float64(v))) ? NaN : Float64(v) for v in tab[!, basemean_col]]
  log2fc = Float64[(ismissing(v) || !isfinite(Float64(v))) ? NaN : Float64(v) for v in tab[!, log2fc_col]]
  pvals = Float64[(ismissing(v) || !isfinite(Float64(v))) ? NaN : Float64(v) for v in tab[!, pval_col]]

  valid_mask = isfinite.(base_means) .& isfinite.(log2fc) .& (base_means .> 0.0)
  g_valid = gene_ids[valid_mask]
  bm_valid = log10.(base_means[valid_mask])
  l_valid = log2fc[valid_mask]
  p_valid = pvals[valid_mask]

  sig_idx = findall(i -> isfinite(p_valid[i]) && p_valid[i] <= p_thresh, 1:length(g_valid))
  ns_idx = findall(i -> !isfinite(p_valid[i]) || p_valid[i] > p_thresh, 1:length(g_valid))

  make_trace(indices, name, color) = Dict(
    "x" => bm_valid[indices],
    "y" => l_valid[indices],
    "text" => ["Gene: $(g_valid[i])<br>log10 BaseMean: $(round(bm_valid[i], digits=2))<br>log2FC: $(round(l_valid[i], digits=3))<br>padj: $(@sprintf("%.2e", p_valid[i]))" for i in indices],
    "mode" => "markers",
    "type" => "scatter",
    "name" => name,
  ...
  traces = [
    make_trace(sig_idx, "Significant (padj <= $(p_thresh))", "#ef4444"),
    make_trace(ns_idx, "Not Significant", "#6b7280")
  ]

  min_x = isempty(bm_valid) ? -1.0 : floor(minimum(bm_valid) - 0.5)
  max_x = isempty(bm_valid) ? 5.0 : ceil(maximum(bm_valid) + 0.5)

  layout = Dict(
    "title" => Dict("text" => title, "font" => Dict("color" => "#f8fafc", "size" => 18)),
    "paper_bgcolor" => "#1e293b",
    "plot_bgcolor" => "#0f172a",
    "xaxis" => Dict("title" => "log10(Base Mean)", "gridcolor" => "#334155", "zerolinecolor" => "#475569", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc")),
    "yaxis" => Dict("title" => "log2 Fold Change", "gridcolor" => "#334155", "zerolinecolor" => "#475569", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc")),
    "hovermode" => "closest",
    "legend" => Dict("font" => Dict("color" => "#f8fafc")),
    "shapes" => [
      Dict("type" => "line", "x0" => min_x, "x1" => max_x, "y0" => 0, "y1" => 0, "line" => Dict("color" => "#38bdf8", "width" => 1.5, "dash" => "dash"))
    ]
  )
...
function export_ma_plot_html(filepath::String, df_or_results; kwargs...)
  content = ma_plot_to_html(df_or_results; standalone=true, kwargs...)
  mkpath(dirname(abspath(filepath)))
  write(filepath, content)
  return filepath
end
...
"""
    dispersion_plot_to_html(dds_or_info; title="Dispersion Estimates Diagnostic", ...)

Generate an interactive Plotly HTML diagnostic plot of dispersion estimates vs mean expression.
"""
function dispersion_plot_to_html(obj;
  title::String="DESeq2 Dispersion Estimates Diagnostic",
  dom_id::String="dispersion-plot-" * string(rand(1000:9999)),
  standalone::Bool=true)
  ...
  if obj isa DESeqDataSet
    means = dds_or_info_means(obj)
    genewise = obj.gene_wise_dispersions !== nothing ? obj.gene_wise_dispersions : Float64[]
    trend = obj.dispersion_fit !== nothing ? obj.dispersion_fit : Float64[]
    map_disp = obj.dispersions !== nothing ? obj.dispersions : Float64[]
  elseif obj isa NamedTuple || obj isa Dict
    means = get(obj, :mean_expression, get(obj, "mean_expression", Float64[]))
    genewise = get(obj, :gene_wise, get(obj, "gene_wise", Float64[]))
    trend = get(obj, :trend, get(obj, "trend", Float64[]))
    map_disp = get(obj, :map, get(obj, "map", Float64[]))
  end

  valid_mask = isfinite.(means) .& (means .> 0.0)
  m_valid = log10.(means[valid_mask])

  traces = Any[]

  if length(genewise) == length(means)
    gw_valid = log10.(max.(genewise[valid_mask], 1e-6))
    push!(traces, Dict(
      "x" => m_valid,
      "y" => gw_valid,
      "mode" => "markers",
      "type" => "scatter",
      "name" => "Gene-wise (MLE)",
      "marker" => Dict("color" => "#64748b", "size" => 4, "opacity" => 0.5)
    ))
  end

  if length(map_disp) == length(means)
    map_valid = log10.(max.(map_disp[valid_mask], 1e-6))
    push!(traces, Dict(
      "x" => m_valid,
      "y" => map_valid,
      "mode" => "markers",
  ...
  if length(trend) == length(means)
    tr_valid = log10.(max.(trend[valid_mask], 1e-6))
    sort_p = sortperm(m_valid)
    push!(traces, Dict(
      "x" => m_valid[sort_p],
      "y" => tr_valid[sort_p],
      "mode" => "lines",
  ...
  layout = Dict(
    ...
    "paper_bgcolor" => "#1e293b",
    "plot_bgcolor" => "#0f172a",
    "xaxis" => Dict("title" => "log10(Mean Normalized Counts)", "gridcolor" => "#334155", "zerolinecolor" => "#475569", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc")),
    "yaxis" => Dict("title" => "log10(Dispersion)", "gridcolor" => "#334155", "zerolinecolor" => "#475569", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc")),
    "hovermode" => "closest",
    "legend" => Dict("font" => Dict("color" => "#f8fafc"))
  )
...
function dds_or_info_means(dds::DESeqDataSet)
  wc = dds.replaced_counts === nothing ? dds.counts : dds.replaced_counts
  nf = _effective_norm_factors(dds)
  ng = length(wc.gene_ids)
  means = zeros(Float64, ng)
  counts_dense = Matrix{Float64}(wc.counts)
  for g in 1:ng
    means[g] = mean((@view counts_dense[g, :]) ./ nf)
  end
  return means
end

function export_dispersion_plot_html(filepath::String, obj; kwargs...)
  content = dispersion_plot_to_html(obj; standalone=true, kwargs...)
  mkpath(dirname(abspath(filepath)))
  write(filepath, content)
  return filepath
end
...
"""
    de_heatmap_to_html(cm, df_or_results; top_n=50, title="Top Differentially Expressed Genes Heatmap", ...)

Generate an interactive Plotly HTML heatmap of Z-score normalized expression for top N DE genes.
"""
function de_heatmap_to_html(cm::CountMatrix, df_or_results;
  top_n::Int=50,
  title::String="Top Differentially Expressed Genes Heatmap",
  dom_id::String="de-heatmap-" * string(rand(1000:9999)),
  standalone::Bool=true)

  tab = df_or_results isa DataFrame ? df_or_results : DataFrame(df_or_results)

  # Sort by padj
  pvals = Float64[(ismissing(v) || !isfinite(Float64(v))) ? 1.0 : Float64(v) for v in tab[!, :padj]]
  perm = sortperm(pvals)

  selected_rows = perm[1:min(top_n, length(perm))]
  selected_genes = String.(tab[selected_rows, :gene_id])

  # Find matching gene indices in CountMatrix
  gene_map = Dict(cm.gene_ids[i] => i for i in 1:length(cm.gene_ids))
  cm_indices = Int[get(gene_map, g, 0) for g in selected_genes]

  valid_idx = findall(>(0), cm_indices)
  cm_indices = cm_indices[valid_idx]
  selected_genes = selected_genes[valid_idx]

  isempty(cm_indices) && return _wrap_html_document("<p style='color:#f8fafc'>No matching genes found for heatmap.</p>", title; standalone=standalone)

  # Extract log2-CPM or log2(counts + 1)
  Y = Matrix{Float64}(cm.counts[cm_indices, :])
  logY = log2.(Y .+ 1.0)

  # Z-score normalize across samples
  z_matrix = zeros(Float64, length(selected_genes), length(cm.sample_ids))
  for g in 1:size(logY, 1)
    row = logY[g, :]
    m = mean(row)
    s = std(row; corrected=false)
    z_matrix[g, :] .= s > 1e-8 ? (row .- m) ./ s : 0.0
  end

  heatmap_trace = Dict(
    "z" => [z_matrix[i, :] for i in 1:size(z_matrix, 1)],
    "x" => cm.sample_ids,
    "y" => selected_genes,
    "type" => "heatmap",
    "colorscale" => "RdBu",
    "reversescale" => true,
    "colorbar" => Dict("title" => "Z-Score", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc"))
  )
  ...
  layout = Dict(
    ...
    "paper_bgcolor" => "#1e293b",
    "plot_bgcolor" => "#0f172a",
    "xaxis" => Dict("title" => "Samples", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc")),
    "yaxis" => Dict("title" => "Genes", "tickfont" => Dict("color" => "#cbd5e1"), "titlefont" => Dict("color" => "#f8fafc"), "autorange" => "reversed")
  )

  clean_dom = _escape_html_text(dom_id)
  html_snippet = """
<div id="$(clean_dom)" style="width:100%; height:600px;"></div>
<script src="https://cdn.plot.ly/plotly-2.27.0.min.js"></script>
<script>
  (function() {
    var data = [$(_to_json_val(heatmap_trace))];
    var layout = $(_to_json_val(layout));
    Plotly.newPlot('$(clean_dom)', data, layout, {responsive: true});
  })();
</script>
...
function export_de_heatmap_html(filepath::String, cm::CountMatrix, df_or_results; kwargs...)
  content = de_heatmap_to_html(cm, df_or_results; standalone=true, kwargs...)
  mkpath(dirname(abspath(filepath)))
  write(filepath, content)
  return filepath
end
